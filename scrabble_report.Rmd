...
Now we're finally ready. Check out Figure \ref{hist} for the distribution of Scrabble scores for valid words that start with 'a' and are less than 10 letters long. Looks like `r with(hist, Score[which.max(Freq)])` points is the most common score for valid Scrabble words beginning with 'a' that are up to 9 letters long.

![A histogram of Scrabble scores for a subset of English words\label{hist}](scrabble_histogram.png)
